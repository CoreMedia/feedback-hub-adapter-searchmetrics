...
class StatusLabel extends Container{
  ...
  set error(value:boolean) { this.#error = value; }

  constructor(config:Config<StatusLabel> = null){
    super( ConfigUtils.apply(Config(StatusLabel, {
           height: 40,
  items:[
    Config(IconDisplayField, { ui:  DisplayFieldSkin.GREEN.getSkin(),
                         scale: "medium",
                         iconCls:  CoreIcons_properties.check_circle,
      plugins:[
        Config(BindPropertyPlugin, { componentProperty: "hidden",
                               bindTo: ValueExpressionFactory.createFromValue(config.error)})
      ]
    }),
    Config(IconDisplayField, { ui:  DisplayFieldSkin.RED.getSkin(),
                         scale: "medium",
