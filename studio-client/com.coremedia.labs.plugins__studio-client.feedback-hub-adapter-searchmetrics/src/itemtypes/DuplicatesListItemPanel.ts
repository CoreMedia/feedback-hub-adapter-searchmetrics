...
interface DuplicatesListItemPanelConfig extends Config<DuplicatesListItemPanelBase> {
  ...
  items:[
    Config(Container, {
      items:[
        Config(IconDisplayField, { tooltip: this.getLabel(config.feedbackItem[FeedbackHubPropertyNames.PROPERTY_HELP]),
                             iconCls:  CoreIcons_properties.help,
          plugins:[
            Config(BindVisibilityPlugin, {
                    bindTo: ValueExpressionFactory.createFromValue(this.getLabel(config.feedbackItem[FeedbackHubPropertyNames.PROPERTY_HELP]))})
          ]
        })
      ],
    ...
    Config(StatusLabel, { flex: 1, text:  this.getStatusText(config), error: this.getStatusError(config)}),
    Config(Container, {
      items:[
      ...
      plugins:[
        Config(BindVisibilityPlugin, { bindTo: this.getDuplicatesExpression(config), transformer: bind(this,this.visibilityTransformer)})
      ],
    ...
    Config(Container, {
      ...
      plugins:[
        Config(BindComponentsPlugin, { valueExpression: this.getDuplicatesExpression(config),
    ...
    Config(Container, {
      items:[
        Config(Button, { ui:  ButtonSkin.SIMPLE_PRIMARY.getSkin(),
